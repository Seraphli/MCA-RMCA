...
public:
  ...
  virtual void updateAllAssignmentHeap(Agent *updatedAgent,
                                       Task *assignedTask){};
  virtual void logAssignmentHandle(Task *task, Agent *a,
                                   AssignmentHeap *new_assignment_heap){};
  virtual bool updateRealPath(Assignment *min_cost_assign, Agent *a,
                              int next_action = 1){};
  virtual void buildAssignmentHeap();
  virtual void printTaskHeap(){};

  TaskAssignment(AgentLoader *agents, TaskLoader *tasks, MapLoaderCost *map,
                 TA_Options ta_opt, options pl_option, bool real_cost,
                 float pl_time_limit);
  bool assignTasks();
  void initializeOneShot();
  void initializeOnline();
  void initializeAssignments();
  void addToUnassigned(Task *t) { t->heap_handle = unassigned_tasks.push(t); }
  void setStartTimestep(int timestep) { start_timestep = timestep; }

  // optimize part
  std::unordered_set<Task *> remove_hist;

  bool
  optimize(std::unordered_set<int> awaiting_tasks = std::unordered_set<int>());
  ...
  bool updateActions(vector<ActionEntry> &actions, Agent *agent, int pick = 0,
                     int drop = -1);
  ...
  int prioritized_planning(vector<PathEntry> &path,
  ...
  void printPath() {
    for (int agent_id = 0; agent_id < agents->agents.size(); agent_id++) {
      cout << "Agent: " << agent_id
           << ", Delay: " << assignments[agent_id].current_total_delay
           << " , Cost: " << assignments[agent_id].path.size() - 1
           << ", plan: ";
      for (auto &entry : assignments[agent_id].path) {
        cout << entry.timeStep << "(" << entry.location / map->cols << ","
             << entry.location % map->cols << ")->";
      }
      cout << endl;
    }
  }
