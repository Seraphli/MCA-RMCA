...
Task::Task(int id, int init_t, int init_loc, int goal_loc)
    : task_id(id), initial_time(init_t), initial_location(init_loc),
      goal_location(goal_loc), start_time(0), goal_time(0), aid(-1) {}

Task::Task(int id, int init_t, int init_loc, int goal_loc, int start_time,
           int goal_time, int aid)
    : task_id(id), initial_time(init_t), initial_location(init_loc),
      goal_location(goal_loc), start_time(start_time), goal_time(goal_time),
      aid(aid) {}
...
TaskLoader::TaskLoader(const std::string fname, MapLoaderCost &ml) {
  ...
  if (myfile.is_open()) {
    ...
    for (int i = 0; i < num_of_tasks; i++) {
      ...
      //      cout << "AGENT" << i << ":   START[" << curr_pair.first << "," <<
      //      curr_pair.second << "] ; ";
...
void TaskLoader::loadKiva(const std::string fname, MapLoaderCost &ml) {
  ...
  for (int i = 0; i < task_num; i++) {
    ...
    new_task->ideal_end_time =
        ml.getDistance(ml.endpoints[s], ml.endpoints[g]) + t;
...
void TaskLoader::loadMAPDwHC(const std::string fname, MapLoaderCost &ml) {
  string line;
  ifstream myfile(fname.c_str());
  if (!myfile.is_open()) {
    assert(false);
  ...
  for (int i = 0; i < task_num; i++) {
    ...
    assert(g < ml.endpoints.size());
    Task *new_task =
        new Task(i, t, ml.endpoints[s], ml.endpoints[g], ts, tg, aid);
    new_task->ideal_end_time =
        ml.getDistance(ml.endpoints[s], ml.endpoints[g]) + t;
