...
TaskAssignment::TaskAssignment(AgentLoader *agents, TaskLoader *tasks,
                               MapLoaderCost *map, TA_Options ta_opt,
                               options pl_options, bool real_cost,
                               float pl_time_limit) {
  this->agents = agents;
  this->tasks = tasks;
  this->map = map;
...
void TaskAssignment::initializeOnline() { initializeAssignments(); }

void TaskAssignment::initializeAssignments() {
  assignments.resize(agents->num_of_agents);
  for (Agent *a : agents->agents) {
    assignments[a->agent_id].actions.push_back(
        ActionEntry(NULL, ACTION_TYPE::START, a->initial_location, 0, 0));
    assignments[a->agent_id].actions.push_back(
        ActionEntry(NULL, ACTION_TYPE::DOCK, a->initial_location, 1, 0));

    assignments[a->agent_id].agent = a;
    assignments[a->agent_id].ideal_cost = 0;
    constraintTable.dock(
        assignments[a->agent_id].actions.back().location, a->agent_id,
        assignments[a->agent_id].actions.back().real_action_time);
  }
}
...
bool TaskAssignment::assignTasks() {
  ...
  while (!unassigned_tasks.empty()) {
    if (screen >= 2) {
      cout << "Selecting Task from " << unassigned_tasks.size() << " tasks."
           << endl;
    }
    ...
    if (!min_cost_assign->path.empty()) {
      ...
      assert(assignments[agent_id].path.size() ==
             assignments[agent_id].path_length);
    ...
    constraintTable.delete_dock(assignments[agent_id].actions.back().location,
                                agent_id);
    constraintTable.dock(min_cost_assign->actions.back().location, agent_id,
                         min_cost_assign->actions.back().real_action_time);

    assignments[agent_id].actions = min_cost_assign->actions;
    assignments[agent_id].ideal_cost = min_cost_assign->ideal_cost;
    assignments[agent_id].current_total_delay =
        min_cost_assign->current_total_delay;
...
bool TaskAssignment::optimize(std::unordered_set<int> awaiting_tasks) {
  ...
  if (ta_option.objective == OBJECTIVE::MAKESPAN) {
    iteration_log.push_back(
        {iteration, 0, runtime / CLOCKS_PER_SEC, current_makespan});

  } else {
    iteration_log.push_back(
        {iteration, 0, runtime / CLOCKS_PER_SEC, current_total_delay});
  }
  if (screen >= 1) {
    cout << "****** Start Anytime Optimization *******" << endl;
    cout << "Iteration: " << iteration << ", select: ";

    cout << ", runtime: " << (runtime - op_start_time) / CLOCKS_PER_SEC
         << ", cost: "
         << ((ta_option.objective == OBJECTIVE::MAKESPAN) ? current_makespan
                                                          : current_total_delay)
         << endl;
  }
  ...
  while (iteration < ta_option.max_iteration &&
         (runtime - op_start_time) / CLOCKS_PER_SEC < ta_option.time_limit) {
    ...
    if (screen >= 1) {
      ...
      cout << ", runtime: " << (runtime - op_start_time) / CLOCKS_PER_SEC
           << ", cost: "
           << ((ta_option.objective == OBJECTIVE::MAKESPAN)
                   ? current_makespan
                   : current_total_delay)
           << endl;
    }
    if (ta_option.objective == OBJECTIVE::MAKESPAN) {
      iteration_log.push_back({iteration, (int)selected.size(),
                               runtime / CLOCKS_PER_SEC, current_makespan});

    } else {
      iteration_log.push_back({iteration, (int)selected.size(),
                               runtime / CLOCKS_PER_SEC, current_total_delay});
    }
  }
...
bool TaskAssignment::removeFromMax(std::unordered_set<int> &awaiting_tasks) {
  ...
  for (int a_id = 0; a_id < assignments.size(); a_id++) {
    ...
    if (assignments[a_id].assigned_tasks.size() > 0 && !all_selected) {
      if ((ta_option.objective == OBJECTIVE::MAKESPAN &&
           assignments[a_id].makespan > max_makespan) ||
          (ta_option.objective != OBJECTIVE::MAKESPAN &&
           assignments[a_id].current_total_delay > max_delay)) {
  ...
  if (screen >= 2)
    cout << "Makespan before remove: " << current_makespan << ", path size "
         << max_assign.makespan << " , actions " << max_assign.actions.size()
         << endl;
  ...
  constraintTable.delete_dock(max_assign.actions.back().location,
                              max_assign.agent->agent_id);
  constraintTable.dock(max_assign.actions.back().location,
                       max_assign.agent->agent_id,
                       max_assign.actions.back().real_action_time);
  ...
  if (screen >= 2)
    cout << "Makespan after remove: " << current_makespan << ", path size "
         << max_assign.makespan << " , actions"
         << assignments[max_id].actions.size() << endl;
  return true;
}
bool TaskAssignment::removeRandom(std::unordered_set<int> &awaiting_tasks) {
  if (screen >= 2)
    cout << "Makespan before remove: " << current_makespan
         << ", Delay before remove " << current_total_delay << endl;
  ...
  for (auto &assign : assignments) {
    ...
    if (removed) {
      ...
      constraintTable.delete_dock(assign.actions.back().location,
                                  assign.agent->agent_id);
      constraintTable.dock(assign.actions.back().location,
                           assign.agent->agent_id,
                           assign.actions.back().real_action_time);
  ...
  if (screen >= 2)
    cout << "Makespan after remove: " << current_makespan
         << ", Delay after remove " << current_total_delay << endl;
...
bool TaskAssignment::removeMultiMax(std::unordered_set<int> &awaiting_tasks) {
  ...
  if (ta_option.objective == OBJECTIVE::MAKESPAN) {
    std::sort(sorted_assigns.begin(), sorted_assigns.end(),
              Assignment::compare_assignment_span);
  } else {
    std::sort(sorted_assigns.begin(), sorted_assigns.end(),
              Assignment::compare_assignment_delay);
  }
...
bool TaskAssignment::selectRemoveRepair(
  ...
  if (remove_success) {
    ...
    if (screen >= 2)
      cout << "Makespan after re-assignment: " << current_makespan
           << ", Delay after re-assignment: " << current_total_delay << endl;
  }
  if (!remove_success ||
      (ta_option.objective != OBJECTIVE::MAKESPAN &&
...
int TaskAssignment::haveConflict(int agent_id, vector<PathEntry> &path) {
  for (int i = 1 + (start_timestep >= 0 ? start_timestep : 0); i < path.size();
       i++) {
    if (constraintTable.is_path_constrained(agent_id, path[i].location,
                                            path[i - 1].location, i)) {
      return i;
    }
  }
...
int TaskAssignment::prioritized_planning(vector<PathEntry> &path,
                                         vector<ActionEntry> &actions, Agent *a,
                                         int next_action) {
  clock_t t = clock();
  num_of_pp += 1;
  ...
  if (next_action > 1) {
    ...
    //            start_time++; //start_timestep is the time_step previous to
    //            current timestep. next_action-1 is a past start action. need
    //            to make sure next_action is excutated after start_timestep
    //        }
    ...
    if (start_timestep > 0) {
      ...
      assert(start_timestep ==
             actions[assignments[a->agent_id].start_action].real_action_time);
      start_loc = path[start_timestep].location;
      start_time = start_timestep;
      total_delay = actions[assignments[a->agent_id].start_action]
                        .real_current_total_delay;
      assert(start_loc ==
             actions[assignments[a->agent_id].start_action].location);
  ...
  for (int i = next_action; i < actions.size(); i++) {
    ...
    if (start_time == start_timestep && min_end_time <= start_timestep) {
      min_end_time =
          start_timestep + 1; // start_timestep is a past previous time_step.
    }
    if (start == goal && start_time >= min_end_time) {
      if (actions[i].action_type == ACTION_TYPE::DROP_OFF &&
          start_time > actions[i].task->ideal_end_time) {
    ...
    if (actions[i].action_type == ACTION_TYPE::DROP_OFF) {
      total_delay +=
          planner.path.back().timeStep - actions[i].task->ideal_end_time;
    }
    ...
    if (!path.empty())
      path.pop_back(); // kick the current start location from existing path's
                       // end.
...
Assignment *TaskAssignment::insertTask(Agent *a, Task *task, bool real_cost) {
  ...
  for (int pick = start_action; pick < assignments[a->agent_id].actions.size();
       pick++) {
    for (int drop = pick + 1;
         drop < assignments[a->agent_id].actions.size() + 1; drop++) {

      vector<ActionEntry> temp_actions = assignments[a->agent_id].actions;

      temp_actions.insert(temp_actions.begin() + pick,
                          ActionEntry(task, ACTION_TYPE::PICK_UP,
                                      task->initial_location,
                                      task->initial_time));
      temp_actions.insert(temp_actions.begin() + drop,
                          ActionEntry(task, ACTION_TYPE::DROP_OFF,
                                      task->goal_location, task->initial_time));
      ...
      cost_increase =
          total_delay - assignments[a->agent_id].current_total_delay;
      if (ta_option.objective == OBJECTIVE::MAKESPAN)
        cost_increase =
            (temp_actions[temp_actions.size() - 2].ideal_action_time -
             current_makespan) > 0
                ? temp_actions[temp_actions.size() - 2].ideal_action_time -
                      current_makespan
                : 0;
      //            if (cost_increase < min_cost_increase && real_cost){
      //                total_delay = prioritized_planning(path,temp_actions,
      //                a); if (total_delay < 0){
      //                    continue;
      //                }
      //                cost_increase = total_delay -
      //                assignments[a->agent_id].current_total_delay;
      //            }

      if (screen > 5) {
        cout << "Actions length: " << temp_actions.size()
             << ",cost_increase: " << cost_increase << ", min increase"
             << min_cost_increase << endl;
      }
      if (cost_increase < min_cost_increase ||
          (cost_increase == min_cost_increase &&
           temp_actions.back().ideal_action_time <
               assignments[a->agent_id].ideal_cost)) {
        ...
        if (screen > 5) {
          cout << "assign to: " << a->agent_id
               << ",cost_increase: " << cost_increase << endl;
        }
    ...
    if (task->aid == -1) {
      for (Agent *a : agents->agents) {
        ...
        Assignment *min_cost_assign =
            insertTask(a, task, ta_option.real_cost_insert);
        if (min_cost_assign->current_total_delay >= 0) {
          min_cost_assign->heap_handle =
              new_assignment_heap->push(min_cost_assign);
          handleTable[task->task_id][a->agent_id] =
              min_cost_assign->heap_handle;
        } else {
...
bool TaskAssignment::updateActions(vector<ActionEntry> &actions, Agent *agent,
                                   int pick, int drop) {
  if (drop == -1) {
    drop = actions.size() - 1;
  }
  ...
  for (int i = pick; i < actions.size(); i++) {
    ...
    int temp_action_time =
        map->getDistance(actions[i - 1].location, actions[i].location) +
        action_time;
    if (actions[i].action_type == ACTION_TYPE::PICK_UP &&
    ...
    if (a.action_type == ACTION_TYPE::DROP_OFF) {
      total_delay =
          total_delay + (a.ideal_action_time - a.task->ideal_end_time);
    ...
    if (a.action_type == ACTION_TYPE::START ||
        a.action_type == ACTION_TYPE::DOCK)
...
void TaskAssignment::printAssignments() {
  for (auto &a : assignments) {
    cout << "Agent: " << a.agent->agent_id << ", Cost: " << a.ideal_cost
         << ", Capacity: " << a.agent->capacity
         << ", Task amount: " << a.assigned_tasks.size() << ", Actions:";
    a.print();
  }
}
